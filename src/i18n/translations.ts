...
const ja: Record<string, string> = {
  ...
  "Keeb-On! Studio is free to use and needs no installation. You can look at a supported keyboard in demo mode before buying it, build a keymap there, and apply it once the keyboard arrives.":
    "この Keeb-On! Studio はインストール不要で無料で使用できます。対応キーボードを購入前でもデモモードで確認でき、キーマップを作成して購入後に適用することもできます。",
  "The latest firmware for every supported keyboard, with the steps for writing it, is on the downloads page.":
    "対応キーボードの最新ファームウェアと書き込み手順は、ダウンロードページにまとめています。",
  "Open the downloads page": "ダウンロードページを開く",
  "Opinions and questions about using it go here":
    "使い方について意見や質問はこちら",
  ...
  "About Keeb-On! Studio":
    "Keeb-On! Studio について（できること・対応キーボード・Q&A）",
  ...
  "Divides the sensor's positional wave out of every report with no lag — the keyboard learns the wave's shape by itself from the first stroke and keeps it. The wave's period in sensor counts, to a tenth, used only while the search above is off. Geometry says resolution ÷ (2 × electrodes along this axis), 76.0 here (1974 ÷ 26), but the equaliser needs it to within a percent — one count off leaves a third of the wave — and of the two pads measured so far one wanted 75.5 and the other had no wave at all. If no convincing wave is learned at this period, nothing is corrected. Set by hand only with the search off: scan half a count at a time with the waveform tool open. 0 turns it off.":
    "センサの位置の波を、遅延なしで毎レポートから割り出して消します。波の形はキーボード自身が最初のストロークで学習し、覚えておきます。値は波の周期(センサのカウント、0.1刻み)で、上の自動探索がオフのときだけ使われます。幾何学的には 解像度 ÷ (2 × その軸の電極本数) = ここでは 76.0(1974 ÷ 26)ですが、補正は周期を1%以内で必要とし(1カウントずれると波の1/3が残る)、実測した2枚のパッドは片方が 75.5、もう片方は波なしでした。この周期で確かな波が学習できなければ何も補正しません。手で決めるときは速度波形ツールを開いたまま 0.5 刻みで走査します。0 で無効。",
  ...
  Shop: "ショップ",
  Community: "コミュニティ",
  "Stored Settings": "保存済み設定",
  "Input Processors": "入力プロセッサ",
...
const zh: Record<string, string> = {
  ...
  "Keeb-On! Studio is free to use and needs no installation. You can look at a supported keyboard in demo mode before buying it, build a keymap there, and apply it once the keyboard arrives.":
    "Keeb-On! Studio 免费使用，无需安装。购买之前就可以在演示模式中查看支持的键盘，先做好键位图，收到键盘后再应用。",
  "The latest firmware for every supported keyboard, with the steps for writing it, is on the downloads page.":
    "所有支持的键盘的最新固件和刷写步骤，都汇总在下载页面。",
  "Open the downloads page": "打开下载页面",
  "Opinions and questions about using it go here":
    "关于用法的意见和问题请到这里",
  ...
  "About Keeb-On! Studio": "关于 Keeb-On! Studio（功能・支持的键盘・问答）",
  "The keyboard refused the value. A sleep timeout needs firmware built with CONFIG_ZMK_SLEEP; without it the keyboard never enters deep sleep.":
  ...
  "Ripple correction, up and down ({{n}} counts)":
    "波纹校正 上下（周期 {{n}} 计数）",
  "Divides the sensor's positional wave out of every report with no lag — the keyboard learns the wave's shape by itself from the first stroke and keeps it. The wave's period in sensor counts, to a tenth, used only while the search above is off. Geometry says resolution ÷ (2 × electrodes along this axis), 76.0 here (1974 ÷ 26), but the equaliser needs it to within a percent — one count off leaves a third of the wave — and of the two pads measured so far one wanted 75.5 and the other had no wave at all. If no convincing wave is learned at this period, nothing is corrected. Set by hand only with the search off: scan half a count at a time with the waveform tool open. 0 turns it off.":
    "无延迟地把传感器位置的波从每次上报中除去——键盘会在第一次划动时自行学习波形并记住。数值是波的周期（传感器计数，精确到 0.1），仅在上方的自动搜索关闭时使用。按几何计算为 分辨率 ÷（2 × 该轴电极数），这里为 76.0（1974 ÷ 26），但校正需要周期精确到 1% 以内（偏差 1 个计数会留下三分之一的波），实测的两块触控板一块需要 75.5，另一块根本没有波。若在此周期上学不到可信的波，则不做任何校正。手动设置时请打开速度波形工具，以 0.5 为步进扫描。0 为关闭。",
  "Ripple correction, left and right ({{n}} counts)":
    "波纹校正 左右（周期 {{n}} 计数）",
  "The same correction for the short axis. Leave it at 0 unless the waveform tool finds a period there too; on this pad it does not.":
  ...
  Shop: "商店",
  Community: "社区",
  "Stored Settings": "已保存的设置",
  "Input Processors": "输入处理器",
