...
const ja: Record<string, string> = {
  ...
  "Divides the sensor's positional wave out of every report with no lag — the keyboard learns the wave's shape by itself from the first stroke and keeps it. The wave's period in sensor counts, to a tenth. Geometry says resolution ÷ (2 × electrodes along this axis), 76.0 here (1974 ÷ 26), but the equaliser needs it to within a percent — one count off leaves a third of the wave — and the two pads measured so far wanted 75.5 and something else, so leave the search above on and this is only where it starts. Set by hand only with the search off: scan half a count at a time with the waveform tool open. 0 turns it off.":
    "センサの位置の波を、遅延なしで毎レポートから割り出して消します。波の形はキーボード自身が最初のストロークで学習し、覚えておきます。値は波の周期(センサのカウント、0.1刻み)。幾何学的には 解像度 ÷ (2 × その軸の電極本数) = ここでは 76.0(1974 ÷ 26)ですが、補正は周期を1%以内で必要とし(1カウントずれると波の1/3が残る)、実測した2枚のパッドは 75.5 と別の値を求めました。なので上の「自動で探す」をオンのままにし、ここは出発点にとどめてください。手で決めるのは自動探索を切ったときだけ: 速度波形ツールを開いたまま 0.5 刻みで走査します。0 で無効。",
  ...
  "The keyboard tries a bank of periods alongside the correction and adopts the one whose wave comes out largest — every pad has its own value, to a tenth, and this saves scanning for it. Takes ten or twenty seconds of ordinary strokes on each pad, is remembered across power cycles, and starts over if the pad scale changes. The periods below are then only the starting point; an axis with no wave never locks and keeps its value. Turn it off to hold the values below exactly.":
    "キーボードが補正と並行して多数の周期候補を試し、波が最も大きく出た周期を採用します。周期はパッドごとに 0.1 単位で違うので、手で走査する手間がなくなります。各パッドで普通に 10〜20 秒なぞれば決まり、電源を切っても覚えています。パッドのスケールを変えると探し直します。下の周期はその出発点になるだけです。波のない軸は決して確定せず、設定値のままです。下の値をそのまま固定したいときはオフに。",
...
const zh: Record<string, string> = {
  ...
  "Ripple correction, up and down ({{n}} counts)":
    "波纹校正 上下（周期 {{n}} 计数）",
  "Divides the sensor's positional wave out of every report with no lag — the keyboard learns the wave's shape by itself from the first stroke and keeps it. The wave's period in sensor counts, to a tenth. Geometry says resolution ÷ (2 × electrodes along this axis), 76.0 here (1974 ÷ 26), but the equaliser needs it to within a percent — one count off leaves a third of the wave — and the two pads measured so far wanted 75.5 and something else, so leave the search above on and this is only where it starts. Set by hand only with the search off: scan half a count at a time with the waveform tool open. 0 turns it off.":
    "无延迟地把传感器位置的波从每次上报中除去——键盘会在第一次划动时自行学习波形并记住。数值是波的周期（传感器计数，精确到 0.1）。按几何计算为 分辨率 ÷（2 × 该轴电极数），这里为 76.0（1974 ÷ 26），但校正需要周期精确到 1% 以内（偏差 1 个计数会留下三分之一的波），而实测的两块触控板分别需要 75.5 和另一个值——所以请保持上方的自动搜索开启，这里只是起点。只有关闭自动搜索时才手动设置：打开速度波形工具，以 0.5 为步进扫描。0 为关闭。",
  "Ripple correction, left and right ({{n}} counts)":
    "波纹校正 左右（周期 {{n}} 计数）",
  "The same correction for the short axis. Leave it at 0 unless the waveform tool finds a period there too; on this pad it does not.":
  ...
  "The keyboard tries a bank of periods alongside the correction and adopts the one whose wave comes out largest — every pad has its own value, to a tenth, and this saves scanning for it. Takes ten or twenty seconds of ordinary strokes on each pad, is remembered across power cycles, and starts over if the pad scale changes. The periods below are then only the starting point; an axis with no wave never locks and keeps its value. Turn it off to hold the values below exactly.":
    "键盘会在校正的同时尝试一组候选周期，并采用波形最大的那个——每块触控板的周期都不同（精确到 0.1），这样就不必手动扫描。在每块触控板上正常划动十到二十秒即可确定，断电后也会记住；触控板比例改变时会重新查找。下方的周期只是起点；没有波的轴永远不会锁定，保持其设定值。若要精确固定下方的值，请关闭。",
