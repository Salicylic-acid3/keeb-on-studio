...
describe("finding a keyboard's trackpad gestures", () => {
  ...
  it("keeps the gesture positions past the end of the physical keys", () => {
    // 72 is the first of the nine pseudo-keys drawn below the board. A
  ...
  it("says nothing when the layout is the wrong size", () => {
    ...
    expect(trackpadGesturesFor(ERGOTRACK.name, 81 - 1)).toEqual([]);
    expect(trackpadGesturesFor(ERGOTRACK.name, undefined)).toEqual([]);
  });

  it("says nothing about a keyboard it does not know", () => {
    expect(trackpadGesturesFor("Someone Else's Board", 81)).toEqual([]);
    expect(trackpadGesturesFor(undefined, 81)).toEqual([]);
    expect(trackpadGesturesFor("", 81)).toEqual([]);
  });
  ...
  it("lists no position twice", () => {
    const positions = trackpadGesturesFor(ERGOTRACK.name, 81).map(
      (g) => g.position,
    );
  ...
  it("covers every gesture position and the unused spare", () => {
    ...
    for (const gesture of trackpadGesturesFor(ERGOTRACK.name, 81)) {
  ...
  it("hides nothing on a layout it cannot vouch for", () => {
    ...
    expect(pseudoKeyPositionsFor(ERGOTRACK.name, 82).size).toBe(0);
    expect(pseudoKeyPositionsFor("Someone Else's Board", 81).size).toBe(0);
  ...
  it("marks the two modifiers as held and the swipes as taps", () => {
    ...
    // a pinch's or a zoom's wheel scroll into a zoom. A tapped one would do
    // nothing.
    const byPosition = new Map(
      trackpadGesturesFor(ERGOTRACK.name, 81).map((g) => [g.position, g]),
    );
    ...
    expect(byPosition.get(79)?.held).toBe(false);
    expect(byPosition.get(80)?.held).toBe(false);
  });
});
